...
data/                        published JSON, read directly by index.html — not hand-edited
  ...
  claim-checks.json          Evidence drawer check results (Cabinet-Level) — generated by publish.py
...
docs/                        site-specific content (separate from the private repo's docs/)
  ...
  tap-source-check-method.html   public methodology page for the Evidence drawer's automated source check (generated from the private repo's tech-report)
...
`index.html` has a per-tracker `evidenceDrawerEnabled` flag in the `TRACKERS` object (Cabinet-Level only; off until the rollout step). The drawer's builders are covered by `tests/evidence-drawer.test.js` and `test_evidence_drawer_browser.py`. It reads `data/claim-checks.json`, which `publish.py` generates from the private repo's `tracker/claim_checks_public.py`; `test_claim_checks_corpus.py` checks the file's invariants against `data/*.json`. The page it links to is `docs/tap-source-check-method.html`.
